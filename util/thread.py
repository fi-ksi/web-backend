...
def to_json(thread, user_id=None, unread_cnt=None, posts_cnt=None):
    if posts_cnt is None:
        posts_cnt = session.query(model.Post).\
            filter(model.Post.thread == thread.id).count()
    if unread_cnt is None:
        if user_id is None:
            unread_cnt = posts_cnt
        else:
            unread_cnt = count_unread(user_id, thread.id)
...
def get_user_visit(user_id, year_id):
    return session.query(model.ThreadVisit).\
        ...
        filter(model.ThreadVisit.user == user_id,
               model.Thread.year == year_id).\
        all()
...
def count_unread(user_id, thread_id):
    ...
    return session.query(model.Post).\
        filter(model.Post.thread == thread_id,
...
def is_eval_thread(user_id, thread_id):
    return session.query(model.SolutionComment).\
        filter(model.SolutionComment.user == user_id,
               model.SolutionComment.thread == thread_id).\
        count() > 0
