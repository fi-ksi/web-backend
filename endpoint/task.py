...
class Task(object):
    ...
    def on_get(self, req, resp, id):
        try:
            ...
            if ((not user.is_logged_in()) or ((not user.is_org()) and
                                              (not user.is_tester()))):
...
class Tasks(object):
    ...
    def on_get(self, req, resp):
        try:
            ...
            fsubmitted = util.task.fully_submitted(
                user.id,
                req.context['year']
            )
...
class TaskDetails(object):
    ...
    def on_get(self, req, resp, id):
        try:
            ...
            threads = [
                session.query(model.Thread).get(thread_id)
                for thread_id in thread_ids if thread_id is not None
            ]
            posts = []
            ...
            req.context['result'] = {
                ...
                'modules': [
                    util.module.to_json(module, user.id)
                    for module in task.modules
                ],
                'moduleScores': [
                    util.module.score_to_json(score)
                    for score in scores if score.points is not None
                ],
                'achievements': [
                ...
                'userScores': [
                    util.task.best_score_to_json(best_score)
                    for best_score in best_scores
                ],
                'threads': [
                    util.thread.to_json(thread, user.id)
                    for thread in threads
                ],
                'threadDetails': [
                    util.thread.details_to_json(thread)
                    for thread in threads
                ],
                'posts': [
                    util.post.to_json(post, user.id)
                    for post in posts
                ]
            }
        except SQLAlchemyError:
